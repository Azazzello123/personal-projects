...
bool ValidirajBrojPasosa(string pasos) {
    return regex_search(pasos, regex("[A-Z]{1,2}[0-9]{3,4}[ -]?[0-9]{2,4}"));
}
...
public:
    Gost(const char* imePrezime, string emailAdresa, string brojPasosa) {
        ...
        ValidirajBrojPasosa(brojPasosa) ? _brojPasosa = brojPasosa : "NOT VALID";
    }
    Gost(const Gost& obj) {
...
class Rezervacija {
    ...
    void posaljimail(Komentar k) {
        ...
        cout << "\n Subject: Informacija" << endl << endl;
        cout << "Postovani," << endl << endl;
        cout << "Zaprimili smo Vase ocjene, a njihova prosjecna vrijednost je " << k.ProsjecnaOcjena() << endl;
        cout << "Zao nam je zbog toga, te ce Vas u najkracem periodu kontaktirati nasa Sluzba za odnose sa gostima." << endl << endl;
        cout << "Ugodan boravak Vam zelimo" << endl << endl;
        cout << "Puno pozdrava" << endl << endl;
        m.unlock();
    }
public:
    ...
    void SetKomentar(Komentar k) {
        _komentar = k;
        int brojac = 0;
        for (size_t i = 0; i < k.GetOcjeneKriterija()->getTrenutno() ; i++)
        {
            if (k.GetOcjeneKriterija()->getElement2(i)<5)
            {
                brojac++;
            }
        }
        if (brojac>=2)
        {
            thread t1(&Rezervacija::posaljimail, this, k);
            t1.join();
        }
    }
    ...
    pair<int, int> GetBrojZnakova(const char* putanja, const char* znakovi) {
        int UkupnoZnakova = 0, UkupnoIstihZnakova = 0;
        ifstream fajl;
        fajl.open(putanja);
        char znak;
        while (fajl.get(znak)) {
        ...
        pair<int, int>novi;
        novi.first = UkupnoZnakova;
        novi.second = UkupnoIstihZnakova;
        fajl.close();
        return novi;
    }
    ~Rezervacija(){}
